...
UnderFlow 是一款基于 Tauri 2 + React 19 构建的轻量级桌面流程图编辑器。它提供直观的拖拽式操作界面，支持多种节点类型、自动布局、颜色定制和多格式导出，适用于快速绘制流程图、架构图、工作流图等可视化文档。
...
| 命令 | 说明 |
|------|------|
| `save_flowchart` | 保存流程图 JSON |
| `load_flowchart` | 加载流程图 JSON |
| `save_file_dialog` | 弹出保存文件对话框 |
| `open_file_dialog` | 弹出打开文件对话框 |
...
|------|------|------|
| Process | 圆角矩形 | 通用处理步骤 |
| Input | 圆角矩形（底部连接点） | 流程起点 |
| Output | 圆角矩形（顶部连接点） | 流程终点 |
...
5. **现代技术栈**：React 19 + Tauri 2 + TypeScript，代码质量高
6. **MIT 开源**：可自由使用、修改和分发
